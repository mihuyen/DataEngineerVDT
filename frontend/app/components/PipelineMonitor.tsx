...
const StatusBadge = ({ status }: { status: string }) => {
  const cfg: Record<string, { color: string; bg: string; icon: React.ReactNode }> = {
    ...
    running: { color: "#f59e0b", bg: "rgba(245,158,11,0.1)", icon: <Loader size={12} /> },
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div>
        ...
        <p style={{ color: "#6b7fa3", margin: 0, fontSize: 12, ...INTER }}>Giám sát pipeline & chất lượng dữ liệu · 14/06/2026 14:30</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12 }}>
        {[
          ...
          { label: "DAG đang chạy", value: runningCount, color: "#f59e0b", bg: "rgba(245,158,11,0.08)" },
          { label: "Lỗi GX Validation", value: totalErrors, color: "#ff4d6d", bg: "rgba(255,77,109,0.08)" },
          { label: "Bản ghi ingest hôm nay", value: "967K", color: "#3b82f6", bg: "rgba(59,130,246,0.08)" },
          { label: "Kafka Consumer Lag", value: "35ms", color: "#00d97e", bg: "rgba(0,217,126,0.08)" },
        ].map((kpi) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
        <ServiceCard name="Kafka" status="healthy" metric="98.4%" icon={<Activity size={14} />} unit="uptime · 35ms lag" />
        <ServiceCard name="ClickHouse" status="healthy" metric="142ms" icon={<Database size={14} />} unit="avg query time" />
        <ServiceCard name="MinIO" status="healthy" metric="2.4 TB" icon={<HardDrive size={14} />} unit="used / 10 TB" />
        <ServiceCard name="Airflow" status="healthy" metric="10/10" icon={<Server size={14} />} unit="DAGs active" />
        <ServiceCard name="Spark" status="healthy" metric="8 cores" icon={<Activity size={14} />} unit="3 workers active" />
      </div>
        ...
        <div style={CARD}>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={kafkaLag} margin={{ left: 10, right: 10 }}>
              ...
              <Line type="monotone" dataKey="lag" stroke="#f59e0b" strokeWidth={2} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
        {[
          { layer: "Bronze (Raw)", used: 850, total: 3000, color: "#f59e0b" },
