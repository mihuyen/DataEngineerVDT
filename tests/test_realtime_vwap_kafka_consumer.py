...
def test_trade_tick_to_kafka_message_formats_fields() -> None:
    message = trade_tick_to_kafka_message(
        "vcb", datetime(2026, 6, 28, 9, 15, 30), 50000.5, 100
    )
    assert message == {
        "ticker": "VCB",
...
def test_run_cycle_returns_zero_for_empty_ticks() -> None:
    client = FakeClickHouseClient(pd.DataFrame(columns=["ticker", "trade_ts", "price", "volume"]), False)
